...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const populateFilters = (data) => {
        ...
        data.forEach(item => {
            // Add fallback in case the word is somehow null
            const validWord = item.correct || item.word || '';
            const firstLetter = validWord.charAt(0);
            ...
            item._banglishSearch = toBanglish(item.correct + " " + item.word);
        });
        ...
        if (data.length === 0) {
            container.innerHTML = `
                <div class="empty-state bn-text">
                    ...
                    <p>আপনার খোঁজার শর্তের সাথে মিলে এমন কোনো শব্দ নেই।</p>
                </div>
            `;
            if (typeof lucide !== 'undefined') lucide.createIcons();
            return;
        ...
        data.forEach(item => {
            html += `
                <div class="word-card">
                    ...
                    <span class="word-correct bn-text">${item.correct}</span>
    ...
    // FETCH UPDATED FILE NAMES
    fetch('../topic-data/spelling/spelling_rules.json')
        .then(res => res.json())
        .then(data => renderRules(data))
        .catch(err => console.error(err));

    fetch('../topic-data/spelling/spelling_words.json')
        .then(res => res.json())
        .then(data => {
